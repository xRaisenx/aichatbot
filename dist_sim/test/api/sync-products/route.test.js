...
const mockUpsertFn = jest.fn(); // Typed mock
const mockFetchFn = jest.fn(); // Typed mock, returns array of fetched items
...
const mockedFetchAdminShopifyProducts = fetchAdminShopifyProducts;
// Helper to create mock product data
const createMockProduct = (id, title) => ({
    id,
    title,
    handle: `handle-${id}`,
    vendor: `Vendor ${id}`,
...
describe.skip('Sync Products API', () => {
    let originalVectorURL;
    let originalVectorToken;
    beforeEach(() => {
    ...
    it('should verify VectorMetadata construction', async () => {
        ...
        const request = { url: 'http://localhost:3000/api/sync-products?secret=test-secret' };
        await GET(request);
        expect(mockUpsertFn).toHaveBeenCalledWith(expect.arrayContaining([
    ...
    it('should fetch one product and push correctly structured data to Upstash Vector for sparse search', async () => {
        ...
        mockProduct.descriptionHtml = 'This is a <b>test description</b> for sparse search.';
        mockedFetchAdminShopifyProducts.mockResolvedValue({
