...
export async function callGeminiForUnderstanding(query, history) {
    if (!query) {
        return null;
    }
...
export async function generateEmbeddings(text) {
    if (!text) {
        console.error('generateEmbeddings: Input text is empty.');
        return null;
    }
    try {
        ...
        if (embedding && embedding.values && Array.isArray(embedding.values) && embedding.values.length === 768) {
            console.log(`Successfully generated ${embedding.values.length}-dimension embedding for text.`);
            return embedding.values;
        }
        else {
            console.error('Failed to generate valid 768-dimension embedding. Embedding:', embedding);
            return null;
        }
    }
    catch (error) {
