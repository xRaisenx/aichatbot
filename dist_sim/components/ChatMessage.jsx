...
export function ChatMessage({ message, onAddToCart }) {
    ...
    if (message.isLoading) {
        return (<div className={`${styles['message-base']} ${styles['bot-message']} ${styles.messageBubble} flex items-center space-x-2 opacity-80`}>
    ...
    if (message.isError) {
        return (<div className={`${styles['message-base']} ${styles['bot-message']} ${styles.messageBubble} bg-red-100 dark:bg-red-900/30 border border-red-300 dark:border-red-700 text-red-700 dark:text-red-300`}>
        <p className="font-medium">Oops!</p>
        {sanitizedText && <div dangerouslySetInnerHTML={{ __html: sanitizedText }}/>}
      </div>);
    }
    // --- Standard User/Bot Message ---
    return (<div className={`${styles['message-base']} ${isUser ? styles['user-message'] : styles['bot-message']} ${styles.messageBubble}`}>
      {!isUser && message.ai_understanding && (null // Keep this as null if you don’t want to display ai_understanding
      ...
      {!isUser && parsedProductCard && (<div className="mt-3 mb-1">
          <ProductCard title={parsedProductCard.title} description={parsedProductCard.description} price={parsedProductCard.price} image={parsedProductCard.image} landing_page={parsedProductCard.landing_page} matches={parsedProductCard.matches} productId={parsedProductCard.variantId} availableForSale={parsedProductCard.availableForSale} quantityAvailable={parsedProductCard.quantityAvailable} onAddToCart={(productId) => onAddToCart(productId, parsedProductCard.title)}/>
        </div>)}

      {!isUser && message.product_comparison && message.product_comparison.length > 0 && (<div className="mt-3 mb-1">
          <ProductComparison products={message.product_comparison}/>
        </div>)}

      {!isUser && message.complementary_products && message.complementary_products.length > 0 && (<div className="mt-3 mb-1">
          <ComplementaryProducts products={message.complementary_products}/>
        </div>)}

      {!isUser && message.knowledge_base_answer && (<div className="mt-3 mb-1">
          <KnowledgeBaseDisplay answer={message.knowledge_base_answer}/>
        </div>)}

      {!isUser && sanitizedAdvice && (<div className="advice-text" dangerouslySetInnerHTML={{ __html: sanitizedAdvice }}/>)}
    </div>);
}
