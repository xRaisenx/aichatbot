...
export function ComplementaryProducts({ products }) {
    if (!products || products.length === 0) {
        return null; // Don't render if no complementary products
    }
    return (<div className="complementary-products-container border-t border-border-light dark:border-border-dark pt-3 mt-3">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <ProductCard key={index} title={product.title} description={product.description} price={product.price} image={product.image} landing_page={product.landing_page} productId={product.variantId} // Pass variantId as productId
        />))}
      </div>
    </div>);
}
