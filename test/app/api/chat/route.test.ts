...
// Simplified compatible type for the mock
type MockGenerateContentResult = {
  response: {
    text: () => string;
  };
};
...
const mockGenerateContent = jest.fn<(prompt: string | (string | import('@google/generative-ai').Part)[]) => Promise<MockGenerateContentResult>>();
...
const mockLibRedisGet = jest.fn(async (userId: string) => {
  const data = redisStore[`user:${userId}:chatHistory`];
  return data ? JSON.parse(data) : null;
});
const mockLibRedisSet = jest.fn(async (userId: string, history: any[]) => {
...
let lruStore: Record<string, any> = {};
jest.mock('lru-cache', () => ({
  __esModule: true,
  LRUCache: jest.fn(() => ({
    get: mockLruCacheGet, 
    set: mockLruCacheSet, 
    delete: jest.fn((key: string) => delete lruStore[key]),
...
describe('/api/chat POST handler', () => {
  beforeEach(() => {
    ...
    mockLibRedisGet.mockImplementation(async (userId: string) => {
        const data = redisStore[`user:${userId}:chatHistory`];
        return data ? JSON.parse(data) : null;
    });
    mockLibRedisSet.mockImplementation(async (userId: string, history: any[]) => {
        redisStore[`user:${userId}:chatHistory`] = JSON.stringify(history);
    });
    
    mockLruCacheGet.mockReturnValue(null); 

    mockGenerateContent.mockImplementation(async (promptArgs: any) => {
      let queryText = '';
      ...
      const fixtureResponse = (geminiResponsesFixture as Record<string, any>)[queryText] || 
                              (geminiResponsesFixture as Record<string, any>)["hello"];
      return { response: { text: () => JSON.stringify(fixtureResponse) } };
    });

    mockVectorQuery.mockImplementation(async (queryArgs: any) => { 
      const keywords = queryArgs.data || "default"; 
      return (vectorSearchResultsFixture as Record<string, any>)[keywords.toLowerCase()] || 
             (vectorSearchResultsFixture as Record<string, any>)["default"] || 
             [];
    });
  ...
  it('should process a simple greeting query using fixtures', async () => {
    ...
    const expectedGeminiResponse = (geminiResponsesFixture as Record<string, any>)['hello'];
  ...
  it('should process a product query using fixtures and return a product card', async () => {
    ...
    const expectedGeminiResponse = (geminiResponsesFixture as Record<string, any>)['i need a vegan lipstick'];
    const expectedVectorProduct = (vectorSearchResultsFixture as Record<string, any>)['vegan lipstick'][0].metadata;
